...
echo "🔧 Setting up APIM environment and running tests..."
...
if ! command -v jq &> /dev/null; then
    ...
    if command -v apt-get &> /dev/null; then
        ...
        echo "⚠️ Please install jq manually to continue"
        exit 1
    fi
fi
...
if [ -z "$APIM_GATEWAY_URL" ] || [ -z "$APIM_SUBSCRIPTION_ID" ]; then
    echo "❌ APIM environment variables not found. Make sure deployment completed successfully."
...
if [ -z "$APIM_SUBSCRIPTION_KEY" ]; then
    echo "❌ Failed to retrieve APIM subscription key"
    exit 1
fi

echo "✅ APIM Subscription Key: ${APIM_SUBSCRIPTION_KEY:0:8}..."

# Create frontend .env file
FRONTEND_ENV_FILE="src/frontend/.env"
echo "📝 Creating frontend environment file: $FRONTEND_ENV_FILE"
...
if python3 test_e2e.py --apim-url "$APIM_GATEWAY_URL" --subscription-key "$APIM_SUBSCRIPTION_KEY"; then
    echo "🎉 All tests passed! Your APIM architecture is working correctly."
    echo ""
    echo "🌐 Access your application at: $APIM_GATEWAY_URL"
    echo "🔑 Use subscription key: $APIM_SUBSCRIPTION_KEY"
    echo ""
    echo "📚 Next steps:"
